...
</code></pre></div><h3 id="migrate" tabindex="-1"><a class="header-anchor" href="#migrate"><span><code>migrate</code></span></a></h3><p><code>migrate</code>命令用于在不同Redis实例之间迁移数据。语法如下：</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>migrate <span class="token function">host</span> port key<span class="token operator">|</span><span class="token string">&quot;&quot;</span> destination-db <span class="token function">timeout</span> <span class="token punctuation">[</span>COPY<span class="token punctuation">]</span> <span class="token punctuation">[</span>REPLACE<span class="token punctuation">]</span> <span class="token punctuation">[</span>AUTH password<span class="token operator">|</span>AUTH2 username password<span class="token punctuation">]</span> <span class="token punctuation">[</span>KEYS key <span class="token punctuation">[</span>key <span class="token punctuation">..</span>.<span class="token punctuation">]</span><span class="token punctuation">]</span>
</code></pre></div><p>参数说明：</p><ul><li><code>host</code>：目标Redis实例IP</li><li><code>port</code>：目标Redis实例端口</li><li><code>key|&quot;&quot;</code>：如果迁移1个键，则写键名；否则，写空字符串</li><li><code>destination-db</code>：目标Redis实例数据库</li><li><code>[COPY]</code>：可选，迁移后不删除源key</li><li><code>[REPLACE]</code>：可选，如果目标Redis实例中存在同名的key，直接替换</li><li><code>[AUTH password|AUTH2 username password]</code>：可选，目标Redis的用户名和密码</li><li><code>[KEYS key [key ...]]</code>：可选，迁移多个键时，指定键列表</li></ul><h2 id="数据持久化机制" tabindex="-1"><a class="header-anchor" href="#数据持久化机制"><span>数据持久化机制</span></a></h2><p>Redis 提供了两种主要的持久化方式，分别是 RDB（Redis DataBase）和 AOF（Append Only File）。这两种方式可以单独或同时使用。</p><h3 id="rdb-保存快照" tabindex="-1"><a class="header-anchor" href="#rdb-保存快照"><span>RDB: 保存快照</span></a></h3><p>RDB（Redis Database），是一种快照持久化方式，将内存中的数据保存到磁盘上的二进制文件中，Redis重启时用RDB文件恢复数据。</p><p>RDB文件保存Redis在某个时间点的数据，其优点是快速、文件紧凑，适合备份大规模数据；缺点是会丢失最后一次快照之后的所有数据。</p><p><strong>配置项</strong></p><div class="language-text" data-ext="text" data-title="text"><pre class="language-text"><code># save 3600 1      # 在3600秒（1小时）内，至少有1个key发生改变
# save 300 100     # 在300秒（5分钟）内， 至少有10个key发生改变
# save 60 10000    # 在60秒内，          至少有10000个key发生改变

dir ./
dbfilename dump.rdb
</code></pre></div><p>配置<code>save</code>配置项后，Redis将会在满足特定条件时自动触发RDB持久化。</p><p>可以通过命令<code>save</code>或<code>bgsave</code>手动触发RDB持久化。</p><p>另外，执行<code>shutdown</code>命令关闭Redis服务器时，也会自动执行<code>bgsave</code>命令</p><h3 id="aof-只追加文件" tabindex="-1"><a class="header-anchor" href="#aof-只追加文件"><span>AOF: 只追加文件</span></a></h3><p>AOF（Append Only File），是一种日志持久化方式，将每一个写命令追加到日志文件末尾，记录数据库的变更记录，Redis重启时重写执行AOF日志文件中的命令来恢复数据。</p><p>开启AOF后，Redis重启时，优先加载AOF持久化文件。</p><p>AOF持久化方式的优点是数据完整性高、适合增量更新；缺点是AOF文件较大、且恢复数据较慢。</p><p><strong>配置项</strong></p><div class="language-text" data-ext="text" data-title="text"><pre class="language-text"><code># 开启AOF
appendonly yes
...
</code></pre></div><p>slave实例a</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token function">mkdir</span> <span class="token parameter variable">-p</span> /docker/redis7a/conf/
...
</code></pre></div><p>slave实例b</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token function">mkdir</span> <span class="token parameter variable">-p</span> /docker/redis7b/conf/
...
</code></pre></div><p>每个Redis实例的基础配置文件如下：</p><div class="language-text" data-ext="text" data-title="text"><pre class="language-text"><code># redis.conf
...
auto-aof-rewrite-percentage 100
auto-aof-rewrite-min-size 64mb
</code></pre></div><h4 id="配置slave实例" tabindex="-1"><a class="header-anchor" href="#配置slave实例"><span>配置slave实例</span></a></h4><p>在2台slave实例的配置文件中，配置master实例的IP、端口、认证信息。</p><div class="language-text" data-ext="text" data-title="text"><pre class="language-text"><code># replicaof &lt;masterip&gt; &lt;masterport&gt;
...
</code></pre></div><h4 id="启动redis实例" tabindex="-1"><a class="header-anchor" href="#启动redis实例"><span>启动Redis实例</span></a></h4><p>1.启动master实例</p><p><code>docker start redis7</code></p><p>2.启动2台slave实例</p><p><code>docker start redis7a redis7b</code></p><p>此时，查看master实例的日志，可以看到，2台slave实例成功连接到了master实例</p><p><code>docker logs redis7</code></p><p><img src="`+o+'" alt=""></p><h4 id="验证" tabindex="-1"><a class="header-anchor" href="#验证"><span>验证</span></a></h4><p>在master实例中，设置键值对&quot;hello=world&quot;</p><p><img src="'+r+'" alt=""></p><p>在2台slave实例中，查看键&quot;hello&quot;的值，成功获取到&quot;world&quot;</p><p><img src="'+c+'" alt=""></p><h3 id="使用命令指定主从关系" tabindex="-1"><a class="header-anchor" href="#使用命令指定主从关系"><span>使用命令指定主从关系</span></a></h3><p>使用<code>slaveof</code>命令，语法如下：</p><p><code>SLAVEOF &lt;host port | NO ONE&gt;</code></p><p>可以临时指定当前Redis实例作为slave</p><p><code>slaveof 172.17.0.2 6379</code></p><p>或者，取消主从关系（成为master）</p><p><code>slaveof no one</code></p><h2 id="哨兵模式" tabindex="-1"><a class="header-anchor" href="#哨兵模式"><span>哨兵模式</span></a></h2><blockquote><p>参考：https://redis.io/docs/management/sentinel/</p></blockquote><p>Redis哨兵（Redis Sentinel）可以监控主从架构中的Redis数据节点实例，当master实例故障时，自动选举1个slave实例，将其升级为新的master实例，实现自动故障发现与转移，确保系统的高可用与容错性。</p><p>Redis本身可以使用sentinel模式运行，支持以下2种启动方式：</p><ul><li><code>redis-sentinel /path/to/sentinel.conf</code></li><li><code>redis-server /path/to/sentinel.conf --sentinel</code></li></ul><p>Redis哨兵数量为奇数，且推荐哨兵数量不少于3个。</p><p>典型的&quot;哨兵+主从复制&quot;拓扑图如下：</p><p><img src="'+i+`" alt=""></p><h3 id="快速开始-1" tabindex="-1"><a class="header-anchor" href="#快速开始-1"><span>快速开始</span></a></h3><h4 id="环境准备-1" tabindex="-1"><a class="header-anchor" href="#环境准备-1"><span>环境准备</span></a></h4><p>使用3个Sentinel节点 + 1个master数据节点 + 2个slave数据节点的架构。</p><p>其中，数据节点使用主从复制时创建的docker容器。</p><p><strong>创建3个docker容器作为Redis哨兵</strong></p><blockquote><p>哨兵节点端口为26379</p></blockquote><p>哨兵实例x</p><div class="language-text" data-ext="text" data-title="text"><pre class="language-text"><code>mkdir -p /docker/redis7-sentinel-a/conf/
...
</code></pre></div><p>哨兵实例y</p><div class="language-text" data-ext="text" data-title="text"><pre class="language-text"><code>mkdir -p /docker/redis7-sentinel-b/conf/
...
</code></pre></div><p>哨兵实例z</p><div class="language-text" data-ext="text" data-title="text"><pre class="language-text"><code>mkdir -p /docker/redis7-sentinel-c/conf/
